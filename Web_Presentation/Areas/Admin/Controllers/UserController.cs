    ...
    public class UserController : Controller
        {
            ...
            [Authorize(Roles = "admin")]
            [HttpPost]
            public async Task<IActionResult> ChangePassword(Entities.Concrete.ChangePassword changePassword)
            {
                var httpClient = _httpClientFactory.CreateClient();
                var token = HttpContext.Session.GetString("Token");
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                ...
                {
                    var responseContent = await responseMessage.Content.ReadAsStringAsync();
                    var apiDataResponse = JsonConvert.DeserializeObject<ApiDataResponse<Entities.Concrete.ChangePassword>>(responseContent);
